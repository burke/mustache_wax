...
class Mustache
  ...
  class Rails < Mustache
    ...
    def render(*args)
      ...
      #puts "%s ---- %s" % [self.class.to_s, params[:format] == 'mustache' && "%s::%s" % [params[:controller].capitalize, params[:action].capitalize] == self.class.to_s]

      if params[:format] == 'mustache'
        #puts "SELF IS A %s %s" % [self.class.name, self.class.name == "%s::%s" % [params[:controller].capitalize, params[:action].capitalize]]
        #debugger

        if self.class.name == "%s::%s" % [params[:controller].capitalize, params[:action].capitalize]
          puts "HIT " + self.class.name
          @@json = {}

          #debugger

          @@mustache_methods.each do |m|
            if method(m).arity == 0
              @@json[m] = send(m)
            end
          end
    ...
    def self.method_added(name)
      if(self.class != 'Mustache::Rails')
        #puts "ADDED %s TO %s" % [name, self.class]
        @@mustache_methods << name
      end
    end
